...
gh secret set SERVER_SSH_PRIVATE_KEY --repo "$REPOSITORY" < "$PRIVATE_KEY"
gh secret set SERVER_SSH_KNOWN_HOSTS --repo "$REPOSITORY" < "$KNOWN_HOSTS"

expected=(SERVER_HOST SERVER_PORT SERVER_USER APP_PATH SERVER_SSH_PRIVATE_KEY SERVER_SSH_KNOWN_HOSTS)
actual="$(gh secret list --repo "$REPOSITORY" --json name --jq '.[].name')"
...
gh workflow run "Deploy production" --repo "$REPOSITORY" -f ref="$ref"
